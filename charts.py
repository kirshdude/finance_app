...
def breakdown(df):
    insight_list = ["Total Expenses Over Time by Category",
                    ...
                    "Breakdown of Category Types and Sub Category Types for a Specific Month",
                    "Total Expenses Over Time by Avoidability",
                    "Total Expenses Over Time by Category and Sub Category"
                    ]
    ...
    def plot_total_expenses_over_time_by_avoidability(df, excluded_months=None):
        ...
        df_monthly_avoidable_sum = df.groupby(['month', 'is_avoidable'])['amount'].sum().reset_index()
        df_monthly_avoidable_sum['month'] = pd.to_datetime(df_monthly_avoidable_sum['month'], format='%Y-%m')
        df_monthly_avoidable_sum = df_monthly_avoidable_sum.pivot(index='month', columns='is_avoidable',
                                                                  values='amount').fillna(0)
        df_monthly_avoidable_sum = df_monthly_avoidable_sum.reset_index()
        df_monthly_avoidable_sum = pd.melt(df_monthly_avoidable_sum, id_vars='month', var_name='is_avoidable',
                                           value_name='amount')
    ...
    # Function to plot total expenses over time by category and sub-category with average line
    def plot_total_expenses_over_time_by_category_and_sub_category(df, selected_categories,
                                                                   selected_sub_categories=None, excluded_months=None):
        df = filter_months(df, excluded_months)

        # Filter by selected categories
        df_filtered = df[df['category'].isin(selected_categories)]

        # Further filter by selected sub-categories, if any
        if selected_sub_categories:
            df_filtered = df_filtered[df_filtered['sub_category'].isin(selected_sub_categories)]

        df_monthly_sum = df_filtered.groupby(['month', 'category'])['amount'].sum().reset_index()
        df_monthly_sum['month'] = pd.to_datetime(df_monthly_sum['month'], format='%Y-%m')

        # Calculate the overall average
        average_total = df_monthly_sum['amount'].mean()

        try:
        # Create the bar chart
            fig = px.bar(df_monthly_sum, x='month', y='amount',
                         color='category' if selected_sub_categories else 'sub_category',
                         labels={'amount': 'Total Expenses'},
                         title=f"Total Expenses Over Time for Selected Categories and Sub Categories",
                         height=700, width=1000)

            # Add a dashed line for the average
            fig.add_shape(type="line",
                          x0=df_monthly_sum['month'].min(), y0=average_total,
                          x1=df_monthly_sum['month'].max(), y1=average_total,
                          line=dict(color="red", width=2, dash="dash"),
                          name="Average Trend")

            # Add annotation for the average line
            fig.add_annotation(x=df_monthly_sum['month'].max(), y=average_total,
            ...
            fig = "Please choose at least one category and sub category."
            return fig
    ...
    elif insight == "Total Expenses Over Time by Category and Sub Category":
        categories = df['category'].unique()
        selected_categories = st.multiselect('Select Categories', categories, default=categories)
        sub_categories = df[df['category'].isin(selected_categories)]['sub_category'].unique()
        selected_sub_categories = st.multiselect('Select Sub Categories (Optional)', sub_categories,
                                                 default=sub_categories)
        fig = plot_total_expenses_over_time_by_category_and_sub_category(df, selected_categories,
                                                                         selected_sub_categories if selected_sub_categories else None,
                                                                         excluded_months)
    try:
        st.write(break_down_charts.format(insight), unsafe_allow_html=True)
        st.plotly_chart(fig)
